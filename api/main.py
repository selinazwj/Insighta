...
def _is_builtin_form_url(form_url: Optional[str], survey_id: Optional[int] = None) -> bool:
    value = (form_url or "").strip()
    if not value or value == "__builtin__":
        return True

    if survey_id is not None:
        legacy_paths = {
            f"/surveys/{survey_id}/take",
            f"surveys/{survey_id}/take",
            f"./surveys/{survey_id}/take",
        }
        if value in legacy_paths:
            return True
        if value.endswith(f"/surveys/{survey_id}/take"):
            return True
...
def _cookie_options(request: Optional[Request] = None, max_age: Optional[int] = None) -> dict:
    secure = False
    if request is not None:
        forwarded_proto = (request.headers.get("x-forwarded-proto") or "").split(",")[0].strip().lower()
        secure = request.url.scheme == "https" or forwarded_proto == "https"
    opts = {"httponly": True, "samesite": "none" if secure else "lax", "secure": secure}
    if max_age is not None:
        opts["max_age"] = max_age
    return opts

def _set_user_cookie(response, request: Request, user_id: int):
    response.set_cookie("user_id", str(user_id), **_cookie_options(request))

def _set_oauth_state_cookie(response, request: Request, state: str):
    response.set_cookie("oauth_state", state, **_cookie_options(request, max_age=300))
...
async def google_callback(
    ...
    _set_user_cookie(resp, request, user.id)
...
def dashboard(
    ...
    for s in matched:
        ...
        is_builtin = _is_builtin_form_url(getattr(s, 'form_url', None), s.id)
        form_link = "" if is_builtin else (s.form_url or "")
...
def get_dashboard_stats(
    ...
    total_earned = getattr(current_user, 'pending_earnings', 0.0) or 0.0
...
def accept_notification(notif_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if r:
        ...
        if participant and survey:
            # Pending earnings are already added when the response is submitted/completed.
            # Approval should not add the reward a second time.
            if getattr(r, "payout_status", None) != "paid":
                r.payout_status = "pending"
            send_email(
                ...
                <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; color: #1a1a18;">
                  ...
                  <div style="background: #f3f1ea; border-radius: 10px; padding: 20px 24px; margin-bottom: 24px;">
                    <div style="font-size: 13px; color: #8a8a82; margin-bottom: 4px;">Survey</div>
                    <div style="font-size: 17px; font-weight: 600; margin-bottom: 12px;">{survey.title}</div>
                    <div style="font-size: 13px; color: #8a8a82; margin-bottom: 4px;">Reward Confirmed</div>
...
def reject_notification(notif_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    survey = db.query(Survey).filter(Survey.id == n.survey_id).first()
    participant = db.query(User).filter(User.id == n.participant_id).first()
    if r and survey and participant and getattr(r, "payout_status", "pending") == "pending":
        participant.pending_earnings = max(0.0, (getattr(participant, 'pending_earnings', 0.0) or 0.0) - (survey.reward_amount or 0.0))
...
async def profile_post(request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not next_url.startswith("/"):
        next_url = "/choice"
    return RedirectResponse(next_url or "/choice", status_code=303)
...
def survey_take(
    ...
    if not survey:
        raise HTTPException(404, "Survey not found")
    return RedirectResponse(url=f"/dashboard?open_survey={survey_id}", status_code=302)
