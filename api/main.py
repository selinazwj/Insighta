...
EMAIL_ADDRESS = os.environ.get("EMAIL_ADDRESS", "insightacom@gmail.com")
EMAIL_PASSWORD = os.environ.get("EMAIL_PASSWORD", "babx pysm yevt tamq")
...
# ---------------------------
# Login / register / password reset helpers
# ---------------------------
    ...
    <div style="font-family: DM Sans, Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; color: #1a1a18;">
      ...
      <div style="background: #f3f1ea; border: 1px solid #e0ddd3; border-radius: 16px; padding: 24px;">
        ...
        <p style="font-size: 13px; line-height: 1.7; color: #8a8a82; margin: 20px 0 0;">
          This code expires in {VERIFICATION_CODE_EXPIRE_MINUTES} minutes. If you did not request this email, you can ignore it safely.
        </p>
      </div>
      <p style="font-size: 12px; color: #8a8a82; margin-top: 20px;">Sent from Insighta official mailbox: {EMAIL_ADDRESS}</p>
    </div>
    '''
...
async def send_auth_code(
    ...
    if normalized_purpose == "reset_password" and not existing_user:
        return JSONResponse({"ok": False, "message": "We could not find an account with this email."}, status_code=400)
...
def password_reset(
    ...
    if not user:
        return _render_login_page(
            request=request,
            reset_error="We could not find an account with this email.",
            reset_open=True,
    ...
    if new_password != confirm_password:
        return _render_login_page(
            request=request,
            reset_error="The two new passwords do not match.",
            reset_open=True,
            reset_email=normalized_email
        )
    ...
    if len(new_password) < 6:
        return _render_login_page(
            request=request,
            reset_error="Your new password must be at least 6 characters long.",
            reset_open=True,
            reset_email=normalized_email
        )
    ...
    if not _consume_verification_code(db, normalized_email, "reset_password", verification_code):
        return _render_login_page(
            request=request,
            reset_error="Invalid or expired verification code.",
            reset_open=True,
            reset_email=normalized_email
        )
...
def show_register(request: Request):
    return _render_register_page(request=request)
...
async def do_register(
    ...
    if not email:
        return _render_register_page(request, error="Email is required.")
    if password != confirm:
        return _render_register_page(request, error="Passwords do not match.", register_email=email)

    password_error = _validate_registration_password(password)
    if password_error:
        return _render_register_page(request, error=password_error, register_email=email)
    if db.query(User).filter(User.email == email).first():
        return _render_register_page(request, error="Email already exists.", register_email=email)
    if not _consume_verification_code(db, email, "register", verification_code):
        return _render_register_page(request, error="Invalid or expired verification code.", register_email=email)
