...
def add_task_type_if_not_exists(conn, is_sqlite):
    if is_sqlite:
        try:
            conn.execute(text("ALTER TABLE surveys ADD COLUMN task_type TEXT DEFAULT 'survey'"))
            conn.commit()
            print("  + surveys.task_type")
        except Exception as e:
            conn.rollback()
            if "duplicate column" in str(e).lower():
                print("  (skip, exists) surveys.task_type")
            else:
                raise
    else:
        conn.execute(text("ALTER TABLE surveys ADD COLUMN IF NOT EXISTS task_type VARCHAR DEFAULT 'survey'"))
        conn.commit()
        print("  + surveys.task_type (if not exists)")

    conn.execute(text("UPDATE surveys SET task_type = 'survey' WHERE task_type IS NULL OR task_type = ''"))
    conn.commit()
...
def main():
    with engine.connect() as conn:
        ...
        add_task_type_if_not_exists(conn, is_sqlite)
